...
episode.json を作成したら、必ず報告の最後に全シーンのプロンプトを1つのコードブロックにまとめて出力する。



出力形式（1つのコードブロックに全シーン）：
...
（scene1 の image\_prompt をそのまま貼る）



\[scene2.png｜役割名\]

（scene2 の image\_prompt をそのまま貼る）



\[scene3.png｜役割名\]

（scene3 の image\_prompt をそのまま貼る）



\[scene4.png｜役割名\]

（scene4 の image\_prompt をそのまま貼る）
...
\- 4シーン全部を1つのコードブロックにまとめる（分割禁止）

\- \[sceneX.png｜役割名\] をラベルとして各プロンプトの直前に付ける

\- JSON内の image\_prompt の文字列を一字一句そのまま出力する（要約・省略禁止）

\- このブロックを丸ごとコピーして画像生成ツールに順番に貼れる形にする
